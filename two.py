...
def on_mouse(event, x, y, flags, param):
    global clicked_point, latest_depth_mm
    ...
    if 0 <= x < rgb_width and 0 <= y < rgb_height:
        ...
        depth_value = latest_depth_mm[y, x]

        if depth_value <= 0:
            print(f"RGB点 ({x}, {y}) 对应深度无效")
        else:
            print(f"RGB点 ({x}, {y}) 对应深度: {depth_value:.1f} mm")


def main():
    global latest_depth_mm, clicked_point
    ...
    print("按 q 退出")
    print("鼠标左键点击左侧 RGB 图像，可输出该点对应深度")
        ...
        while True:
            ...
            if clicked_point is not None:
                ...
                if 0 <= cx < WIDTH and 0 <= cy < HEIGHT:
                    ...
                    if latest_depth_mm is not None and cy < latest_depth_mm.shape[0] and cx < latest_depth_mm.shape[1]:
                        d = latest_depth_mm[cy, cx]
                        text = f"({cx},{cy}) {d:.1f} mm" if d > 0 else f"({cx},{cy}) invalid"
                        cv2.putText(
                            color_bgr,
                            text,
                            (min(cx + 10, WIDTH - 250), max(cy - 10, 25)),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.6,
                            (0, 0, 255),
                            2,
